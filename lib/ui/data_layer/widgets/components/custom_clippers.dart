...
class CustomAmissaClipper extends CustomClipper<Path>{
  @override
...
class AmissaBigClipper extends CustomClipper<Path>{
  @override
  Path getClip(Size size) {
    Path path = Path();
    final double _xScaling = size.width / 414;
...
class ClipWithShadowPath extends StatelessWidget{
  final BoxShadow shadow;
  final CustomClipper<Path> clipper;
  final Widget child;

  const ClipWithShadowPath({Key? key, required this.shadow, required this.clipper, required this.child}):super(key: key);
